...
public class SecurityServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute("questionOne", sb.getQuestion().get(0));
            request.setAttribute("questionTwo", sb.getQuestion().get(1));
            request.setAttribute("anwserOne", sb.getAnwser().get(0));
            request.setAttribute("anwserTwo", sb.getAnwser().get(1));
            request.setAttribute("messageError", message);
            request.getRequestDispatcher("/confirmSecurity.jsp").forward(request, response);
