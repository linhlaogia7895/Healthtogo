...
//             }
	...
	PatientInfoDomain pi=pib.getPatientInfo(username);
        if(pi.getFIRST_NAME() != null )
            request.setAttribute("firstname", pi.getFIRST_NAME()); 
        else
            request.setAttribute("firstname", "a");
	if(pi.getLAST_NAME() != null)
            request.setAttribute("lastname", pi.getLAST_NAME());
        else
            request.setAttribute("lastname", "a");
        if(pi.getLAST_NAME()!= null)
            request.setAttribute("lastname", pi.getLAST_NAME());
        else
            request.setAttribute("lastname", "a");
        ...
        if(pi.getHOME_PHONE()!= null)
            request.setAttribute("homephone", pi.getHOME_PHONE());
        else
            request.setAttribute("homephone", "");
        if(pi.getCELL_PHONE()!= null)
            request.setAttribute("cellphone", pi.getCELL_PHONE());
        else
            request.setAttribute("cellphone", "");
        if(pi.getDATE_OF_BIRTH()!= null)
            request.setAttribute("date", pi.getDATE_OF_BIRTH());
        else
            request.setAttribute("date", "");
        if(Character.toString(pi.getGENDER()) != null)
            request.setAttribute("gender", Character.toString(pi.getGENDER()));
        else
            request.setAttribute("gender", "");
        if(pi.getCONTRACT()!= null)
