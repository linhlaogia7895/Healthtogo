...
public class PatientBean{
    ...
    /**
     * Method to create invoice for patient
     * @param invoiceName
     * @param invoiceDate
     * @param invoiceAmount
     * @return true if the invoice created, other wise false
     * @throws SQLException 
     */
     @Lock(LockType.WRITE)
   ...
   /**
    * Method to get a specific list of patient record using the patient id
    * @param patientId the patient id
    * @return list of record in database
    */
    @Lock(LockType.WRITE)
    public final ArrayList<Record> getRecordTable(String patientId) {
    ...
    /**
     * method to add new patient journal
     * @param activityUser user who do the function
     * @param username patient username
     * @param purpose purpose of the that journal
     * @param current the current weight
     * @param goal the goal weight
     * @param start the start weight
     * @param lowest the lowest weight
     * @param halfway the halfway weight
     * @param lost weight lost
     * @param calories the calories need
     * @param chest chest measurement
     * @param waist waist measurement
     * @param abdomen abdomen measurement
    ...
    /**
     * Method to check completion for a patient journal
     * @param activityUser user who does the function
     * @param theUsername patient username
     * @param data boolean value represent complete or incomplete
   ...
   /**
    * Method to get invoice list for a patient
    * @param username patient username
    * @return list of invoice
    */
    @Lock(LockType.WRITE)
    public final ArrayList<Invoice> getInvoiceTable(String username) {
	ArrayList<Invoice> fc=new ArrayList<>();
	
	try {
	    PreparedStatement ps;
	    ResultSet rs;
	    try (Connection conn = getConnection()) {
		String sql="{call getInvoiceForUser(?)}";
		ps = conn.prepareStatement(sql);
                ps.setString(1, username);
		rs = ps.executeQuery();
                while(rs.next())
		...
		while(rs.previous()){
		    fc.add(Invoice.getInstance(rs.getInt(1),
                            rs.getInt(2),
			    rs.getDate(3),
			    rs.getDouble(4),
			    rs.getDouble(5),
    ...
    /**
     * method to change invoice status to paid 
     * @param username patient username
     * @param id invoice id
     * @return true if successfuly changed, otherwise false
     */
@Lock(LockType.WRITE)
    public final boolean changeInvoiceStatus(String username, String id) 
    {
        boolean complete=false;
        ...
        try {
            PreparedStatement ps;
            try (Connection conn = getConnection()) {
                String sql="{call changeInvoiceStatus(?,?)}";
                ps = conn.prepareStatement(sql);
                ps.setString(1, username);
