 ...
 */
function initMap() {
    var uluru = {lat: 51.1045636, lng: -113.9722315};
 ...
 */
function modalContactPopupSidebox(){
    $('#sec-contact-popup').click(function(){
        $('#modal-contact-popup').modal();
    });
}

/**
 * Function to adjust the sideboxes
 */
function adjustSideBoxes(){
//    var boxHeight = ($('#main-img').height()-10) / 2;
    var boxHeight = $('.all-pages').height()+10;
...
function hoverText(i){
    ...
    if(gifVisible){
        if(canSeeInfo){
            ...
            $(infoE).show(170).text('Click above for information');    
        ...
        if(canSeeInfo){
            ...
            $(infoE).show(170).text('Click above to restore');    
        }
    }
}
